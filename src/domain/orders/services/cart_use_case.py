...
from src.domain.orders.schemas.orders import OrderCreate, OrderRead, OrderStatusUpdateSchema, OrderStatus


class OrderServiceImpl:
    def __init__(self, orders_factory_repository: OrderRepositoryProtocol):
        self.orders_factory_repository: OrderRepositoryProtocol = orders_factory_repository
    ...
    async def get_order_by_user_id(self, user_id: int, order_id: int) -> OrderRead:
        return await self.orders_factory_repository.get_order_by_user_id(user_id, order_id)
    ...
    async def update_status(self, order_id: int, status_order: OrderStatus) -> OrderRead:
        return await self.orders_factory_repository.update_status(order_id, status_order)