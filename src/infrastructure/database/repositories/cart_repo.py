...
class CartRepositoryImpl:
    ...
    async def add(self, user_id: int, item: CartItemCreate) -> CartItemSchema:
        stmt = sa.insert(self.model).values(user_id=user_id, **item.model_dump()).options(selectinload(self.model.picture)).returning(self.model)
    ...
    async def remove_item(self, item_id: int, user_id: int) -> CartItemSchema:
        stmt = sa.delete(self.model).where(self.model.id==item_id, self.model.user_id==user_id).options(selectinload(self.model.picture)).returning(self.model)
        execute = await self.session.execute(stmt)
        result = execute.scalar_one()
