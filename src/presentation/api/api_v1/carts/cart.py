...
async def read_cart(
    svc: CartsService, user: UsersSchemaAuth = Depends(get_current_auth_user)
) -> list[CartItemRead]:
    return await svc.get_cart_by_user(user.id)


@router.post(
    "/cart", response_model=CartItemSchema, status_code=status.HTTP_201_CREATED
)
...
async def clear_cart_user(
    service: CartsService, user: UsersSchemaAuth = Depends(get_current_auth_user)
) -> CartItemRead:
